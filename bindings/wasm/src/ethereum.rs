...
pub struct EthContract {
    contract: defi_wallet_core_common::EthContract,
}
...
impl EthContract {
    ...
    pub fn new(abi_contract: &str) -> Result<Self, JsValue> {
        Self {
            contract: defi_wallet_core_common::EthContract::new(abi_contract),
        }
    }
    ...
    pub fn encode(&self, function_name: &str, args: Vec<JsValue>) -> Result<Vec<u8>, JsValue> {
        let tokens = args
            .into_iter()
            .map(|val| val.try_into().map(|arg| arg.token))
            .collect()?;
...
impl EthContractFunctionArg {
    ...
    pub fn build_address(address_str: &str) -> Result<JsValue, JsValue> {
        JsValue::from_serde(&Self {
            token: EthToken::build_address(address_str),
        })
    ...
    pub fn build_fixed_bytes(bytes: Vec<u8>) -> Result<JsValue, JsValue> {
        JsValue::from_serde(&Self {
            token: EthToken::FixedBytes(bytes),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
    ...
    pub fn build_int(int_str: &str) -> Result<JsValue, JsValue> {
        JsValue::from_serde(&Self {
            token: EthToken::build_int(&int_str),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
    ...
    pub fn build_uint(uint_str: &str) -> Result<JsValue, JsValue> {
        JsValue::from_serde(&Self {
            token: EthToken::build_uint(&uint_str),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
    ...
    pub fn build_fixed_array(values: Vec<JsValue>) -> Result<JsValue, JsValue> {
        let tokens: Vec<EthToken> = values
            .into_iter()
            .map(|val| val.try_into().map(|arg| arg.token))
            .collect()?;
        JsValue::from_serde(&Self {
            token: EthToken::FixedArray(tokens.into_iter().map(Into::into).collect()),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
    ...
    pub fn build_array(values: Vec<JsValue>) -> Result<JsValue, JsValue> {
        let tokens: Vec<EthToken> = values
            ...
            .collect()?;
        JsValue::from_serde(&Self {
            token: EthToken::Array(tokens.into_iter().map(Into::into).collect()),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
    ...
    pub fn build_tuple(values: Vec<JsValue>) -> Result<JsValue, JsValue> {
        let tokens: Vec<EthToken> = values
            ...
            .collect()?;
        JsValue::from_serde(&Self {
            token: EthToken::Tuple(tokens.into_iter().map(Into::into).collect()),
        })
        .map_err(|e| JsValue::from_str(&format!("error: {e}")))
    }
}
