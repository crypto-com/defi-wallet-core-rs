...
pub struct Client {
    ...
    pub client: QueryClient<tonic::transport::Channel>,
}
...
impl Client {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    /// Supply queries the total supply of a given denom or owner
    pub fn supply_blocking(&mut self, denom_id: String, owner: String) -> Result<u64, RestError> {
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.supply(denom_id, owner).await
        })
    }

    /// Owner queries the NFTs of the specified owner
    pub async fn owner(&mut self, denom_id: String, owner: String) -> Result<Option<Owner>, RestError> {
    ...
    pub fn owner_blocking(
        ...
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.owner(denom_id, owner).await
        })
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    /// Collection queries the NFTs of the specified denom
    pub fn collection_blocking(&mut self, denom_id: String) -> Result<Option<Collection>, RestError> {
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.collection(denom_id).await
        })
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    /// Denom queries the definition of a given denom
    pub fn denom_blocking(&mut self, denom_id: String) -> Result<Option<Denom>, RestError> {
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.denom(denom_id).await
        })
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    /// DenomByName queries the definition of a given denom by name
    pub fn denom_by_name_blocking(&mut self, denom_name: String) -> Result<Option<Denom>, RestError> {
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.denom_by_name(denom_name).await
        })
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    /// Denoms queries all the denoms
    pub fn denoms_blocking(&mut self) -> Result<Vec<Denom>, RestError> {
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.denoms().await
        })
    ...
    pub fn nft_blocking(
        ...
        let rt = tokio::runtime::Runtime::new().map_err(|_err| RestError::AsyncRuntimeError)?;
        rt.block_on(async move {
            self.nft(denom_id, token_id).await
        })
    }
}
